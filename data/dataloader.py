...
class TumorCystBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for _ in range(self.num_batches):
            ...
            cyst_batch = rng.choice(self.cyst_indices, size=self.num_cyst_per_batch, replace=True)

            tumor_batch = rng.choice(self.tumor_indices, size=self.num_tumor_per_batch, replace=True)

            if self.num_other_per_batch > 0:
                other_batch = rng.choice(self.other_indices, size=self.num_other_per_batch, replace=True)
